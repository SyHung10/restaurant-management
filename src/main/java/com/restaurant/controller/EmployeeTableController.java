...
public class EmployeeTableController {
    ...
    // 2. Chọn bàn, hiển thị menu và thông tin bàn
    ...
    public String showMenu(@PathVariable("tableId") Long tableId, Model model) {
        ...
        java.math.BigDecimal hourDiscount = java.math.BigDecimal.ZERO;
        ...
        if (session != null) {
            ...
            for (Order o : sessionOrders) {
                sessionTableIds.add(o.getTableId()); // Collect unique table IDs
            ...
            // Tính khuyến mãi giờ vàng nếu có
            if (session.getPromotionId() != null) {
                // Note: Cần inject PromotionService để tính toán khuyến mãi
                // Tạm thời để hourDiscount = 0
            }
        ...
        model.addAttribute("hourDiscount", hourDiscount);
        model.addAttribute("finalAmount", paymentTotal.subtract(hourDiscount));
    ...
    public String addAdditionalTables(
        ...
        try {
            ...
            for (Long additionalTableId : additionalTableIds) {
                ...
                // Chỉ cho phép thêm bàn AVAILABLE hoặc RESERVED, không thêm bàn đang SERVING hoặc đã thanh toán hoặc đang dọn
                ...
                // Tạo order mới cho bàn phụ với cùng session_id
                Order newOrder = new Order();
                newOrder.setSessionId(session.getSessionId());
                newOrder.setTableId(additionalTableId);
