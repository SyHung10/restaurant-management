...
public class KitchenController {
    ...
    public String cancelDetail(@PathVariable("id") Long detailId,
            ...
            @RequestParam(value = "redirectQuery", required = false) String redirectQuery) {
        OrderDetail detail = orderDetailService.findById(detailId);
        ...
        if (redirectQuery != null && !redirectQuery.isEmpty()) {
    ...
    public String kitchenKanban(
        ...
        for (Order o : allOrders) {
            ...
            // Skip orders with null order time
            if (o.getOrderTime() == null) {
                continue;
            }

            // Check from date
            if (from != null && o.getOrderTime().compareTo(from) < 0) {
                includeOrder = false;
            }

            // Check to date
            if (to != null && o.getOrderTime().compareTo(to) > 0) {
                includeOrder = false;
            }
