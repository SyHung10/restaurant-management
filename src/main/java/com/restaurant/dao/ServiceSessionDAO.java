...
public class ServiceSessionDAO {
    ...
    public ServiceSession findActiveByTableId(Long tableId) {
        Session s = sessionFactory.openSession();
        try {
            return (ServiceSession) s.createQuery("from ServiceSession where tableId = :tableId and status = 'ACTIVE'")
    ...
    public List<ServiceSession> findByTableId(Long tableId) {
        Session s = sessionFactory.openSession();
        try {
            return s.createQuery("from ServiceSession where tableId = :tableId")
                    .setParameter("tableId", tableId)
                    .list();
        } finally {
            s.close();
