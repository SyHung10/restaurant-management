    ...
    <div class="container mt-3">
        ...
        <tbody>
          <c:forEach var="odv" items="${orderDetailViews}">
            <tr>
              ...
              <td>
                <c:choose>
                  <c:when test="${odv.menu.category == 'APPETIZER_AND_SIDE'}"
                    >Khai vị & Ăn kèm
                  </c:when>
                  <c:when test="${odv.menu.category == 'BEEF'}">Thịt bò</c:when>
                  <c:when test="${odv.menu.category == 'PORK'}"
                    >Thịt heo</c:when
                  >
                  <c:when test="${odv.menu.category == 'SEAFOOD'}"
                    >Hải sản</c:when
                  >
                  <c:when
                    test="${odv.menu.category == 'RICE_AND_SOUP_AND_NOODLES'}"
                    >Cơm & Canh & Mỳ
                  </c:when>
                  <c:when test="${odv.menu.category == 'HOT_POT'}">Lẩu</c:when>
                  <c:when test="${odv.menu.category == 'DRINK'}"
                    >Đồ uống</c:when
                  >
                  <c:when test="${odv.menu.category == 'DESSERT'}"
                    >Tráng miệng</c:when
                  >
              ...
              <td>
                ...
                <c:if test="${not empty odv.orderDetail.cancelReason}">
                  <p class="text-danger">
                    Lý do hủy: ${odv.orderDetail.cancelReason}
                  </p>
                </c:if>
              </td>
            </tr>
      ...
      <div class="mt-3">
        <a
          ...
          >Quay lại danh sách bàn</a
        >
        <a
          href="${pageContext.request.contextPath}/employee/table/${tableId}/menu"
