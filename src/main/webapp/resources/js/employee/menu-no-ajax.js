...
function addToCart(dishId, name, price) {
    ...
    const button = document.querySelector(`.add-to-cart-btn[data-id="${dishId}"]`);
    if (button) {
...
function removeFromCart(index) {
    ...
    const itemElements = document.querySelectorAll('#cart-items-list .order-item');
...
function editItemQuantity(index) {
    const item = cartItems[index];
    const newQuantity = prompt(`Số lượng mới cho món ${item.name}:`, item.quantity);
...
function submitOrder() {
    ...
    const existingInputs = form.querySelectorAll('input[name="dishIds"], input[name="quantities"]');
...
// Chỉnh sửa số lượng món đã đặt - sử dụng form submit thay vì AJAX
function editExistingItemQuantity(orderDetailId, itemName, currentQuantity) {
    const newQuantity = prompt(`Số lượng mới cho món ${itemName}:`, currentQuantity);
        ...
        if (!isNaN(quantity) && quantity >= 0) {
            if (quantity === 0) {
                ...
                if (reason && reason.trim()) {
                    ...
                    form.method = 'POST';
                    form.action = contextPath + '/employee/orderDetail/' + orderDetailId + '/cancel';
                ...
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = contextPath + '/employee/orderDetail/' + orderDetailId + '/updateQuantity';
...
$(document).ready(function() {
    ...
    const existingOrdersContent = document.getElementById('existing-orders-content');
